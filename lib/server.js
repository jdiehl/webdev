...
module.exports = function (config, done) {
  ...
  if (config.log) {
    ...
    if (typeof config.log !== 'object') config.log = { format: 'common' };
    config.log.options = config.log.options || {};
    if (config.log.path) config.log.options.stream = fs.createWriteStream(config.log.path, {flags: 'a'});
